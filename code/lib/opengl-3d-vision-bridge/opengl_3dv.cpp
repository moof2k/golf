...
void WindowsAssertf(const char *file, int line, const char *pszFormat, ...)
{
	va_list args;
	char text[512];
...
void WindowsPrintf(const char *pszFormat, ...)
{
	va_list args;
	char text[512];
...
#define D3D_EXCEPTION(M) WindowsAssertf(__FILE__, __LINE__, "Error: %s\n", M);
#define D3D_EXCEPTIONF(F, ...) WindowsAssertf(__FILE__, __LINE__, "Error: " F " %s\n", __VA_ARGS__);
 ...
 */
void GLD3DBuffers_create(GLD3DBuffers *gl_d3d_buffers, void *window_handle, int fullscreen, int vsync, int stereo) {
	...
	if (fullscreen) {
		WindowsPrintf(" We are in fullscreen mode: %lu\n", hWnd);
	}
	else {
		WindowsPrintf(" We are in windowed mode\n");
	}
	...
	*/
	...
	WindowsPrintf(" Linked to WGL API\n");
	...
	WindowsPrintf(" WGL NV_DX_interop supported on this platform\n");
	...
	if (wddm) {
		WindowsPrintf(" This operating system uses WDDM (it's Windows Vista or later)\n");
	}
	else {
		WindowsPrintf(" This operating system does not use WDDM (it's prior to Windows Vista)\n");
	}
	...
	if (wddm) {
		...
		WindowsPrintf(" Activated Direct3D 9Ex (WDDM)\n");
	}
	else {
		...
		WindowsPrintf(" Activated Direct3D 9 (no WDDM)\n");
	}
	...
	WindowsPrintf(" Retrieved adapter display mode, format: %u\n", d3dBufferFormat);
	...
	WindowsPrintf(" Our required formats are supported\n");
	...
	if (gl_d3d_buffers->stereo) {
		...
		WindowsPrintf(" Initialized NV API:\n");
	...
	WindowsPrintf(" Enabled OpenGL interop on device\n");
	...
	if (gl_d3d_buffers->stereo && wddm) {
		...
		WindowsPrintf("  Separation: %f\n", separation);
		WindowsPrintf("  Convergence: %f\n", convergence);
		switch (frustum) {
		case NVAPI_NO_FRUSTUM_ADJUST:
			WindowsPrintf("  Frustum: no adjust\n");
			break;
		case NVAPI_FRUSTUM_STRETCH:
			WindowsPrintf("  Frustum: stretch\n");
			break;
		case NVAPI_FRUSTUM_CLEAR_EDGES:
			WindowsPrintf("  Frustum: clear edges\n");
			break;
		default:
			WindowsPrintf("  Frustum: unknown\n");
		}
		WindowsPrintf("  Eye separation: %f\n", eyeSeparation);
	}
	...
	WindowsPrintf(" Retrieved device back buffer\n");
	...
	WindowsPrintf(" Created color texture (left)\n");
	...
	if (gl_d3d_buffers->stereo) {
		...
		WindowsPrintf(" Created color texture (right)\n");
	}
	...
	WindowsPrintf(" Retrieved color buffer from color texture (left)\n");
	...
	if (gl_d3d_buffers->stereo) {
		...
		WindowsPrintf(" Retrieved color buffer from color texture (right)\n");
	}
	...
	if (gl_d3d_buffers->stereo) {
		...
		result = d3dDevice->CreateRenderTarget(
			...
			NULL);
		/*result = d3dDevice->CreateOffscreenPlainSurface(
			width * 2, height + 1, // the extra row is for the stereo header
			d3dBufferFormat,
			D3DPOOL_DEFAULT,
			&d3dStereoColorBuffer,
		...
		WindowsPrintf(" Created render target surface (stereo)\n");
		...
		WindowsPrintf(" Inserted stereo header into render target surface (stereo)\n");
	}
	...
	WindowsPrintf(" Created depth/stencil surface\n");
	...
	WindowsPrintf(" Created OpenGL color texture (left)\n");
	...
	if (gl_d3d_buffers->stereo) {
		...
		WindowsPrintf(" Created OpenGL color texture (right)\n");
	}
	...
	WindowsPrintf(" Created OpenGL depth/stencil texture\n");
	...
	WindowsPrintf(" Bound render target surface to OpenGL texture (left)\n");
	...
	if (gl_d3d_buffers->stereo) {
		...
		WindowsPrintf(" Bound render target surface to OpenGL texture (right)\n");
	}
	...
	WindowsPrintf(" Bound depth/stencil surface to OpenGL render buffer\n");
...
void GLD3DBuffers_flush(GLD3DBuffers *gl_d3d_buffers) {
	...
	if (result != D3D_OK) {
		static int mode_error_countdown = 60;
		...
		{
			mode_error_countdown--;

			if (mode_error_countdown == 0)
				D3D_EXCEPTION("S_PRESENT_MODE_CHANGED counter");
